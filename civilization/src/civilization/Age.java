...
public class Age {
	 ...
	 * @param scierie
	 */
	public void upgradeBatiment(Plateau plateau, Maison maison, Mine mine,
			Carriere carriere, Ferme ferme, Scierie scierie) {
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
	 ...
	 * @param scierie
	 */
	public void evolutionBatiment(Maison maison, Mine mine, Carriere carriere,
			Ferme ferme, Scierie scierie) {
	...
	public void newAge(Plateau plateau, Special special, Age age, Bonus bonus,
			Ressources ressources, Maison maison, Mine mine, Carriere carriere,
			Ferme ferme, Scierie scierie) {
		Scanner sc = new Scanner(System.in);
		String r;
		System.err.println("Payer" + getNage() * 300
				+ " de chaque ressources pour avancer à l'age suivant ?");
		System.err
				.println("Tous vos batiment de production auront +10 de capacite et vos maisons pouront accueillir 5 habitant de plus");
		System.out.println("Tapez oui ou non");
		r = sc.nextLine();

		if (r.equals("oui")) {
			if (ressources.getgold() >= getNage() * 300
					&& ressources.getwood() >= getNage() * 300
					&& ressources.getfood() >= getNage() * 300
					&& ressources.getrock() >= getNage() * 300) {
				...
				System.err.println("Felicitation vous avez atteint l'"
						+ getAge() + "!");
				System.err
						.println("Vous avez débloqué 1 nouveau personnage spécial!");
